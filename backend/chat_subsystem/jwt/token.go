...
// Verify checks token.
func (manager *tokenManager) Verify(accessToken string) (chat_subsystem.UserClaims, error) {
	token, err := jwt.ParseWithClaims(
		...
		func(token *jwt.Token) (interface{}, error) {
			...
			if time.Now().Unix() > claim.ExpiresAt {
				return nil, chat_subsystem.ErrExpiredClaims
			}
...
func NewTokenManager(
		secretKey string) chat_subsystem.TokenManager {
	return &tokenManager{
		secretKey: secretKey,
	}
}
